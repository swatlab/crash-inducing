...
df.fp <- as.data.frame(read.csv(file='error_table.csv'), header=TRUE)
df <- merge(df.pred, df.fp, by='revision')


metrics = c('experience', 'message_size', 'closeness', 'changed_file', 'loc', 'insertion', 'deletion', 'unique_types', 'entropy')
